...
export const getAdminStats = async (req: Request, res: Response) => {
  try {
    const { year, period, startDate, endDate } = req.query as { year?: string; period?: string; startDate?: string; endDate?: string };
    const week = period; // Alias for backward compatibility in logic
    const now = new Date();
    const start = startDate ? new Date(startDate) : null;
    const end = endDate ? new Date(endDate) : null;
    const dateRangeFilter = start && end && !Number.isNaN(start.getTime()) && !Number.isNaN(end.getTime())
      ? { gte: start, lte: end }
      : undefined;
    ...
    const sellerIds = topSellerEarnings.map(s => s.userId);
    let bestSellersWithEarnings: {
      id: string;
      name: string;
      avatar: string | null;
      sold: number;
    ...
    if (sellerIds.length > 0) {
      const [sellers, soldCounts, availableCounts, offerCounts] = await Promise.all([
        prisma.user.findMany({
          where: { id: { in: sellerIds }, role: Role.SELLER },
        ...
        prisma.car.groupBy({
          by: ['sellerId'],
          where: { sellerId: { in: sellerIds }, status: CarStatus.AVAILABLE },
          _count: { _all: true }
        }),
      ...
      const soldMap = new Map(soldCounts.map(s => [s.sellerId, s._count._all]));
      const availableMap = new Map(availableCounts.map(s => [s.sellerId, s._count._all]));
      const offersMap = new Map(offerCounts.map(s => [s.sellerId, s._count._all]));

      bestSellersWithEarnings = topSellerEarnings.map(entry => {
        const seller = sellerMap.get(entry.userId);
        if (!seller) return null;
        return {
          id: seller.id,
          name: `${seller.firstName} ${seller.lastName}`,
          avatar: seller.avatar,
          sold: soldMap.get(seller.id) || 0,
          unsold: availableMap.get(seller.id) || 0,
          offers: offersMap.get(seller.id) || 0,
          earnings: Number(entry._sum.amount || 0)
        };
      }).filter(Boolean) as {
        id: string;
        name: string;
